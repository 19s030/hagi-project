...
from image_change import mosic_change, art_change, dot_change, illust_change, hair_change, skin_change,  hair_change2
import main
import shutil
import json
import os
...
def handle_send_message7(event,reply,userid,color):
    hair_change2.hair_image2(event,userid,color)
    reply = str(reply)
    print("通過チェック画像：{}".format(main.FQDN + "/static/" + userid + "/" + event + "_face.jpg"))
    message = []
    message.append(TextSendMessage(text = "画像を加工中です..."))
